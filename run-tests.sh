...
function simpletest_script_execute_batch($test_id, $test_classes) {
  ...
  while (!empty($test_classes) || !empty($children)) {
    while (count($children) < $args['concurrency']) {
      ...
      timer_start('run-tests-' . $test_class);
    ...
    foreach ($children as $cid => $child) {
      ...
      if (empty($status['running'])) {
        ...
        timer_stop('run-tests-' . $children['class']);
    ...
    if (isset($results_map[$result->status])) {
      if ($result->test_class != $test_class) {
        ...
        if (!isset($xml_files[$test_class])) {
          ...
          $end = timer_stop('run-tests-' . $test_class);
          $root = $doc->createElement('testsuite');
          $root->setAttribute('name', $test_class);
          $root->setAttribute('tests', count($results));
          $root->setAttribute('time', $end['time'] / 1000);
      ...
      elseif ($result->status == 'exception') {
        ...
        $out = $dom_document->createElement('system-out');
        $txt = $dom_document->createTextNode($result->message);
        $out->appendChild($txt);
